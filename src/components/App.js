...
function App() {
  ...
  const onImageClick = (e) =>{
    ...
    console.log(planets)
    const match = planets.filter(planet => planet.englishName === selectedPlanet)  //Display detailed planet information on-click
  ...
  const getPlanets = async () => {
    ...
    setPlanets(planets.bodies.filter
      (planet => planet.isPlanet && planet.name.indexOf('1') === -1 && planet.name.indexOf('Pluto') === -1))  //Fetch Planet Data from API
  }
  ...
  useEffect(() => {
    const getImages = async () => {
      ...
      for await (const planet of planets) {
        const data = `/images/${planet.englishName}.png`  //Load images from folder on component mount
        allImages.push(data)
      }
      setImages(allImages)
    }
